...
class alex256SlotLowLfInterface final : public indexInterface<KEY_TYPE, PAYLOAD_TYPE> {
public:
...
public:
  alex_256slot_lowlf::Alex<KEY_TYPE, PAYLOAD_TYPE, alex_256slot_lowlf::AlexCompare,
...
private:
  ...
  void print_key_cmp_distribution(std::string s) {
    std::ofstream out("alex_256slot_lowlf_" + s + "_key_cmp_distribution.log");
    if (!out.is_open()) {
  ...
  void print_key_cmp_stats(std::string s) {
    std::ofstream out("alex_256slot_lowlf_" + s + "_key_cmp_stats.log");
    if (!out.is_open()) {
      std::cerr << "Failed to open file." << std::endl;
      return;
    }
...
template <class KEY_TYPE, class PAYLOAD_TYPE>
void alex256SlotLowLfInterface<KEY_TYPE, PAYLOAD_TYPE>::bulk_load(
    std::pair<KEY_TYPE, PAYLOAD_TYPE> *key_value, size_t num, Param *param) {
  index.bulk_load(key_value, (int)num);
}

template <class KEY_TYPE, class PAYLOAD_TYPE>
bool alex256SlotLowLfInterface<KEY_TYPE, PAYLOAD_TYPE>::get(KEY_TYPE key, PAYLOAD_TYPE &val,
                                                Param *param) {
#ifdef PROFILING
...
template <class KEY_TYPE, class PAYLOAD_TYPE>
bool alex256SlotLowLfInterface<KEY_TYPE, PAYLOAD_TYPE>::put(KEY_TYPE key,
                                                PAYLOAD_TYPE value,
                                                Param *param) {
...
template <class KEY_TYPE, class PAYLOAD_TYPE>
bool alex256SlotLowLfInterface<KEY_TYPE, PAYLOAD_TYPE>::update(KEY_TYPE key,
                                                   PAYLOAD_TYPE value,
                                                   Param *param) {
...
template <class KEY_TYPE, class PAYLOAD_TYPE>
bool alex256SlotLowLfInterface<KEY_TYPE, PAYLOAD_TYPE>::remove(KEY_TYPE key, Param *param) {
  auto num_erase = index.erase(key);
  return num_erase > 0;
}

template <class KEY_TYPE, class PAYLOAD_TYPE>
size_t alex256SlotLowLfInterface<KEY_TYPE, PAYLOAD_TYPE>::scan(
