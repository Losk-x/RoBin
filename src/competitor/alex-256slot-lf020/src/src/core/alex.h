...
class Alex {
 ...
 public:
  ...
  struct Params {
    ...
    // Maximum node size, in bytes. Fixed to 256 slots for this competitor.
    // Higher values result in better average throughput, but worse tail/max
    // insert latency
    int max_node_size = 256 * sizeof(V);
  ...
  struct DerivedParams {
    // The defaults here assume the fixed 256-slot node size.
    int max_fanout = 256;
    int max_data_node_slots = 256;
  };
...
// node's parent.
 ...
 public:
  ...
  void bulk_load(const V values[], int num_keys) {
    ...
    if (cur_root != nullptr) {
      std::ifstream in("alex_256slot_lf020_insert_root.log");
      if (!in.is_open()) {
        std::ofstream out("alex_256slot_lf020_insert_root.log");
        out << "num_inserts,model_slope,model_intercept,num_slots" << std::endl;
      }
      std::ofstream out("alex_256slot_lf020_insert_root.log", std::ios::app);
    ...
    // AlexNode<T, P>* cur_root = root_node_;
    // if (cur_root != nullptr) {
    //   std::ofstream out("alex_256slot_lf020_insert_root.log", std::ios::binary | std::ios::app);
 ...
 public:
  ...
  void print_depth_stats(std::string s) const {
    ...
    std::ofstream out_dist("alex_256slot_lf020_" + s + "_depth_distribution.log");
    if (!out_dist.is_open()) {
  ...
  // get tree depth
  size_t get_alex_256slot_lf020_depth() const {
    if (root_node_ == nullptr) {
      return 0;
    }
  ...
  void print_model_stats(std::string s) const {
    std::ofstream out_file("alex_256slot_lf020_" + s + "_model_stats.log");
    if (!out_file.is_open()) {
        std::cerr << "Failed to open file." << std::endl;
        return ;
    }
  ...
  void print_hist_model_stats(std::string s) {
    std::ofstream out_file("alex_256slot_lf020_" + s + "_hist_model_stats.log");
    if (!out_file.is_open()) {
        std::cerr << "Failed to open file." << std::endl;
        return ;
    }
  ...
  void print_information_gain(std::string s) {
    ...
    std::ofstream hist_out_file("alex_256slot_lf020_" + s + "_hist.log");
  ...
  void print_node_size(std::string s) {
    std::ofstream out_file("alex_256slot_lf020_" + s + "_node_size.log");
    if (!out_file.is_open()) {
        std::cerr << "Failed to open file." << std::endl;
        return ;
    }
  ...
  void print_size_stats(std::string s) const {
    ...
    std::ofstream out("alex_256slot_lf020_" + s + "_size_stats.log");
    if (!out.is_open()) {
        std::cerr << "Failed to open file." << std::endl;
        return;
    }
  ...
  void print_level_model_stats(std::string s) const {
    std::ofstream out_file("alex_256slot_lf020_" + s + "_level_model_stats.log");
    if (!out_file.is_open()) {
        std::cerr << "Failed to open file." << std::endl;
        return ;
    }
  ...
  void print_smo_stats(std::string s) {
    std::ofstream out("alex_256slot_lf020_" + s + "_smo_stats.log");
    if (!out.is_open()) {
      std::cerr << "Failed to open file." << std::endl;
      return;
    }
  ...
  void print_memory_exception(std::string s) {
    std::ofstream out("alex_256slot_lf020_" + s + "_memory_exception.log");
    if (!out.is_open()) {
        std::cerr << "Failed to open file." << std::endl;
        return;
    }
...
#ifdef PROFILING
    std::ofstream out("alex_256slot_lf020_" + s + "_smo_affected_items.log");
    if (!out.is_open()) {
        std::cerr << "Failed to open file." << std::endl;
        return;
    }
