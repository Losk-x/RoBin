...
class LIPP256SlotInterface final : public indexInterface<KEY_TYPE, PAYLOAD_TYPE> {
public:
...
private:
  lipp_256slot::LIPP<KEY_TYPE, PAYLOAD_TYPE> lipp;
};

template <class KEY_TYPE, class PAYLOAD_TYPE>
void LIPP256SlotInterface<KEY_TYPE, PAYLOAD_TYPE>::bulk_load(
    std::pair<KEY_TYPE, PAYLOAD_TYPE> *key_value, size_t num, Param *param) {
  lipp.bulk_load(key_value, static_cast<int>(num));
}

template <class KEY_TYPE, class PAYLOAD_TYPE>
bool LIPP256SlotInterface<KEY_TYPE, PAYLOAD_TYPE>::get(KEY_TYPE key, PAYLOAD_TYPE &val,
                                                Param *param) {
  bool exist;
...
template <class KEY_TYPE, class PAYLOAD_TYPE>
bool LIPP256SlotInterface<KEY_TYPE, PAYLOAD_TYPE>::put(KEY_TYPE key,
                                                PAYLOAD_TYPE value,
                                                Param *param) {
...
template <class KEY_TYPE, class PAYLOAD_TYPE>
bool LIPP256SlotInterface<KEY_TYPE, PAYLOAD_TYPE>::update(KEY_TYPE key,
                                                   PAYLOAD_TYPE value,
                                                   Param *param) {
...
template <class KEY_TYPE, class PAYLOAD_TYPE>
bool LIPP256SlotInterface<KEY_TYPE, PAYLOAD_TYPE>::remove(KEY_TYPE key, Param *param) {
  return lipp.remove(key);
}

template <class KEY_TYPE, class PAYLOAD_TYPE>
size_t LIPP256SlotInterface<KEY_TYPE, PAYLOAD_TYPE>::scan(
